...
policy = PlanningBasedPolicy(planner_type=planner_type, goal=env.goal, conc=env.cfg.concentration)
...
for eps_idx in range(total_episodes):
    ...
    while True:
        ...
        action = policy.act(ship_pos=info['state'], observation=observation, obstacles=obstacles)
